    ...
    public function callApiWithoutEcho(string $prompt, string $systemPrompt): ?array {
        ...
        while ($retryCount <= $this->maxRetries && !$success) {
            if ($retryCount > 0) {
                ...
                echo "Retrying Mistral request ($retryCount/$this->maxRetries) after {$sleepTime}s delay...\n";
                sleep($sleepTime);
            }
        ...
        if (!$success) {
            echo "Failed to get response from Mistral after $this->maxRetries retries.\n";
            return null;
        }
